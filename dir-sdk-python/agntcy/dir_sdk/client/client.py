...
class Client:
    ...
    def verify(
        ...
        if self.config.docker_config:
            ...
            self.config.docker_config.mounts.append(f"type=bind,src={output_path},dst=/{basename}")
            _output_path = basename

        try:
            provider: sign_v1.VerifyRequestProvider = req.provider
            if provider is None:
                self._verify_with_any(req.record_ref, None, _output_path)
            elif provider.HasField("key"):
                self._verify_with_key(req.record_ref, provider.key, _output_path)
            elif provider.HasField("oidc"):
                self._verify_with_oidc(req.record_ref, provider.oidc, _output_path)
            elif provider.HasField("any"):
                self._verify_with_any(req.record_ref, provider.any, _output_path)
            else:
                self._verify_with_any(req.record_ref, None, _output_path)
    ...
    def _verify_with_any(
        ...
        try:
            # Build base command
            command = [*self.config.get_dirctl(), "verify", record_ref.cid, "--output-file", output_path]
    ...
    def _sign_with_oidc(
        ...
        try:
            ...
            # Build base command
            command = [*self.config.get_dirctl(), "sign", record_ref.cid]
